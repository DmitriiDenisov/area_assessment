...
model = cnn_v3()
# model.summary()
model.load_weights('weights/sakaka_cnn_v3_w_5.h5')
...
valid_sat_files = filenames_in_dir(dir_valid_sat, endswith_='.tif')[2:3]
valid_map_files = filenames_in_dir(dir_valid_map, endswith_='.tif')[2:3]
for i, (f_sat, f_map) in enumerate(list(zip(valid_sat_files, valid_map_files))):
    ...
    plot_img_mask(img_sat_[margin_hor: -margin_hor, margin_vert: -margin_vert],
                  map_pred[margin_hor: -margin_hor, margin_vert: -margin_vert],
                  name='VALID_IMG{}_OVERLAY_cnn_v3_stepsize{}'.format(i + 1, step_size),
                  overlay=True, alpha=0.3, show_plot=False, save_output_path='../../plots/sakaka4/')

    plot_img(img_sat_[margin_hor: -margin_hor, margin_vert: -margin_vert],
