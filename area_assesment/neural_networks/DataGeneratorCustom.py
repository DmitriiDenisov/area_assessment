...
class DataGeneratorCustom:
    def __init__(self, batch_size: int, train_dir: str, train_masks_dir: str, train_nokia_poly: str, step_size: int,
                 patch_size: tuple, target_channels: int,
                 nokia_map=False):
    ...
    def __iter__(self):
        while True:
            ...
            for f in file_names:
                ...
                # f_image = f_image / 255
                f_mask = np.array(
                    load_img(join(self.train_masks_dir, '{}_MAP.tif'.format(f[:-4])), color_mode='grayscale'))
                ...
                for i in range(0, len(x_train_batch), self.batch_size):
                    if self.nokia_map:
                        yield [x_train_batch[i: i + self.batch_size].astype(np.float) / 255,
                               x_nokia_batch[i: i + self.batch_size].astype(np.float)], y_train_batch[
    ...
    def get_rotations(self, batch: np.array, rotation_list: list) -> np.array:
        """
        Rotates images in batch and returns rotated images
        :param batch: numpy array, batch of images
        :param rotation_list: list, list of angles
        :return: numpy array, with number of images equal to len(rotation_list) * batch.shape[0]
        """
    ...
    def get_step_pe_epoch(self) -> int:
        """
        Function for total number of steps per epoch
        :return: int, number of steps per one epoch
        """
        total = 0
...
if __name__ == '__main__':
    ...
    while True:
        ...
        for (a, c) in gen:
            ...
            just_show_numpy_as_image((c[0] * 255).reshape(patch_size), type='not_RGB',
                                     name='trash/map_im_{}.tif'.format(i))
            i += 1
            if i > 10:
                1 / 0
