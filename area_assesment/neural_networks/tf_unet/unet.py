...
from area_assesment.neural_networks.tf_unet.layers import *
...
def create_conv_net(x, keep_prob, channels, n_class, layers=3, features_root=16, filter_size=3, pool_size=2, summaries=True):
    ...
    for layer in range(0, layers):
        ...
        if layer == 0:
            w1 = weight_variable_devonc([filter_size, filter_size, channels, features], stddev)
        else:
            w1 = weight_variable_devonc([filter_size, filter_size, features//2, features], stddev)
            
        w2 = weight_variable_devonc([filter_size, filter_size, features, features], stddev)
    ...
    for layer in range(layers-2, -1, -1):
        ...
        w1 = weight_variable_devonc([filter_size, filter_size, features, features//2], stddev)
        w2 = weight_variable_devonc([filter_size, filter_size, features//2, features//2], stddev)
    ...
    # Output Map
    weight = weight_variable_devonc([1, 1, features_root, n_class], stddev)
