...
def unet_circle_farms(patch_height, patch_width, n_ch):
    inputs = Input((patch_height, patch_width, n_ch))

    pool0 = AveragePooling2D(pool_size=4)(inputs)

    conv1 = Convolution2D(32, 3, 3, activation='relu', border_mode='same')(pool0)
    conv1 = Dropout(0.2)(conv1)
    conv1 = Convolution2D(32, 3, 3, activation='relu', border_mode='same')(conv1)
    pool1 = MaxPooling2D(pool_size=(2, 2))(conv1)
    #
    ...
    # #
    conv6 = Convolution2D(2, 1, 1, activation='relu', border_mode='same')(conv5)
    conv6 = core.Reshape((patch_height//4 * patch_width//4, 2))(conv6)
    # ############
    conv7 = core.Activation('softmax')(conv6)
    conv7 = core.Permute((2, 1))(conv7)
    conv7 = core.Reshape((patch_height//4, patch_width//4, 2))(conv7)

    up3 = UpSampling2D(size=(4, 4))(conv7)

    model = Model(input=inputs, output=up3)
    # sgd = SGD(lr=0.01, decay=1e-6, momentum=0.3, nesterov=False)
